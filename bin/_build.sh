...
# Fix wheel filenames.
uv run --no-dev --with tyro --with wheel-filename --with change-wheel-version bin/fix_wheel.py -i "${OUTPUT_DIR}"/*.whl --version="${PACKAGE_VERSION}" --local-version="cu${CUDA_NAME}.torch${TORCH_NAME}"
